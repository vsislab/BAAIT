...
class _ConvBnReLU(nn.Sequential):
    ...
    def __init__(
        self, in_ch, out_ch, kernel_size, stride, 
        ...
        self.add_module("bn", 
            _BATCH_NORM(out_ch, eps=1e-5, momentum=1 - 0.999))
...
class _ResLayer(nn.Sequential):
    ...
    def __init__(self, n_layers, in_ch, out_ch, 
        stride, dilation, multi_grids=None):
...
class _ASPP(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, rates):
        ...
        self.stages.add_module(
            "c0", _ConvBnReLU(in_ch, out_ch, 1, 1, 0, 1))
        for i, rate in enumerate(rates):
            self.stages.add_module(
                "c{}".format(i + 1),
                _ConvBnReLU(
                    in_ch, out_ch, 3, 1, padding=rate, dilation=rate),
            )
        self.stages.add_module("imagepool", _ImagePool(in_ch, out_ch))

    def forward(self, x):
        return torch.cat(
            [stage(x) for stage in self.stages.children()], dim=1)
...
class DeepLabV3(nn.Sequential):
    ...
    def __init__(self, n_classes, n_blocks, atrous_rates, multi_grids, output_stride):
        ...
        self.add_module(
            "layer2", _ResLayer(
                n_blocks[0], ch[0], ch[2], s[0], d[0]))
        self.add_module(
            "layer3", _ResLayer(
                n_blocks[1], ch[2], ch[3], s[1], d[1]))
        self.add_module(
            "layer4", _ResLayer(
                n_blocks[2], ch[3], ch[4], s[2], d[2]))
        self.add_module(
            "layer5", _ResLayer(
                n_blocks[3], ch[4], ch[5], s[3], d[3], multi_grids)
        )
